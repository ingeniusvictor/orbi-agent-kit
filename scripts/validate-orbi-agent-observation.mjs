...
const allowedTop = new Set(['schemaVersion','status','summary','nextActions','artifacts','evidence','authorityImpact','humanApprovalRequired','localEvidenceRequired','recovery']);
...
if (!file) fail('usage: node scripts/validate-orbi-agent-observation.mjs <observation.json>');
let value;
if (file) {
...
if (value && process.exitCode !== 1) {
  ...
  if (value.schemaVersion !== 'orbi.agent.observation.v1') fail('schemaVersion must be orbi.agent.observation.v1');
  ...
  if (Array.isArray(value.evidence)) for (const e of value.evidence) {
    ...
    const keys = Object.keys(e); for (const k of keys) if (!['kind','reference','verification'].includes(k)) fail(`unknown evidence field: ${k}`);
  ...
  if (!value.authorityImpact || typeof value.authorityImpact !== 'object' || Array.isArray(value.authorityImpact)) fail('authorityImpact must be an object');
  else {
    const entries=Object.entries(value.authorityImpact);
    if (!entries.length) fail('authorityImpact requires at least one explicit field');
    for (const [k,v] of entries) if (typeof v !== 'boolean') fail(`authorityImpact.${k} must be boolean`);
  }
  for (const k of ['humanApprovalRequired','localEvidenceRequired']) if (k in value && typeof value[k] !== 'boolean') fail(`${k} must be boolean`);
  ...
  if (process.exitCode !== 1) console.log('VALID orbi.agent.observation.v1');
}
